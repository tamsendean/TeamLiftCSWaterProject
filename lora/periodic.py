...
recv_data = bytearray() #create empty byte array to aggregate data for sending

#create a spacer (3 bytes of all 0's) to break up each individual packet
spacer = bytearray(3)

#accepts sensor data as a byte array
def append_sensordata(dataa):
    if data is not None:
        recv_data.extend(dataa)#append to the data array
        recv_data.extend(spacer)#append spacer to break up data
...
#function thaat actually sends the data to the other lora!
def sendSensorData(sensor_data):
    packet = None
...
#main function that periodically transmits data to LoRa.
while True:
    # Send data here
    if len(recv_data) > 0:
        recv = sendSensorData(recv_data)

        #if the data was sent, wipe the old byte array
        if recv == True:
            recv_data = bytearray()
